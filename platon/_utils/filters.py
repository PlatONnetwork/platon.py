...
def construct_event_filter_params(
    event_abi: ABIEvent,
    abi_codec: ABICodec,
    contract_address: Optional[Bech32Address] = None,
...
class Filter:
    ...
    def __init__(self,
                 filter_id: HexStr,
                 platon_module: "Platon") -> None:
        self.platon_module = platon_module
    ...
    def get_new_entries(self) -> List[LogReceipt]:
        log_entries = self._filter_valid_entries(self.platon_module.get_filter_changes(self.filter_id))
        return self._format_log_entries(log_entries)

    def get_all_entries(self) -> List[LogReceipt]:
        log_entries = self._filter_valid_entries(self.platon_module.get_filter_logs(self.filter_id))
        return self._format_log_entries(log_entries)
...
class LogFilter(Filter):
    ...
    def set_data_filters(self, data_filter_set: Collection[Tuple[TypeStr, Any]]) -> None:
        ...
        if any(data_filter_set):
            self.data_filter_set_function = match_fn(self.platon_module.codec, data_filter_set)
...
def normalize_data_values(type_string: TypeStr, data_value: Any) -> Any:
    ...
    platon-abi v1 returns utf-8 bytes for string values.
    This can be removed once platon-abi v2 is required.
    """
...
class _UseExistingFilter(Exception):
    """
    Internal exception, raised when a filter_id is passed into w3.platon.filter()
    """
    def __init__(
        self,
