...
class GoPlatonAdminModuleTest:
    def test_add_peer(self, web3: "Web3") -> None:
        result = web3.gplaton.admin.add_peer(
    ...
    def test_admin_datadir(self, web3: "Web3", datadir: str) -> None:
        result = web3.gplaton.admin.data_dir()
        assert result == datadir

    def test_admin_node_info(self, web3: "Web3") -> None:
        result = web3.gplaton.admin.node_info()
    ...
    def test_admin_peers(self, web3: "Web3") -> None:
        enode = web3.gplaton.admin.node_info()['enode']
        web3.gplaton.admin.add_peer(enode)
        result = web3.gplaton.admin.peers()
        assert len(result) == 1

    def test_admin_start_stop_rpc(self, web3: "Web3") -> None:
        stop = web3.gplaton.admin.stop_rpc()
        assert stop is True

        start = web3.gplaton.admin.start_rpc()
        assert start is True

    def test_admin_start_stop_ws(self, web3: "Web3") -> None:
        stop = web3.gplaton.admin.stop_ws()
        assert stop is True

        start = web3.gplaton.admin.start_ws()
        assert start is True
