...
from platon._utils.math import (
    percentile,
)
...
def _get_avg_block_time(w3: Web3, sample_size: int) -> float:
    latest = w3.platon.get_block('latest')
    ...
    oldest = w3.platon.get_block(BlockNumber(latest['number'] - constrained_sample_size))
...
def _get_weighted_avg_block_time(w3: Web3, sample_size: int) -> float:
    latest_block_number = w3.platon.get_block('latest')['number']
    constrained_sample_size = min(sample_size, latest_block_number)
    if constrained_sample_size == 0:
        raise ValidationError('Constrained sample size is 0')

    oldest_block = w3.platon.get_block(BlockNumber(latest_block_number - constrained_sample_size))
    ...
    for i in range(oldest_block_number + 1, latest_block_number + 1):
        curr_timestamp = w3.platon.get_block(BlockNumber(i))['timestamp']
...
def _get_raw_miner_data(
    ...
    for _ in range(sample_size - 1):
        ...
        block = w3.platon.get_block(block['parentHash'], full_transactions=True)
