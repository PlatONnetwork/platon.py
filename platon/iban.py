...
class IsValid:
    ...
    def validate(iban_address: Any) -> bool:
        ...
        if re.match(r"^XE[0-9]{2}(PLATON[0-9A-Z]{13}|[0-9A-Z]{30,31})$", iban_address) and \
...
class Iban:
    ...
    @staticmethod
    def fromAddress(address: Union[Address, Bech32Address]) -> "Iban":
        """
        This method should be used to create
        an iban object from platon address
    ...
    def createIndirect(options: IbanOptions) -> "Iban":
        ...
        @return {Iban} the IBAN object
        """
        return Iban.fromBban("PLATON" + options["institution"] + options["identifier"])
    ...
    def address(self) -> Union[str, Bech32Address]:
        """
        ...
        if self.isDirect():
            base36 = self._iban[4:]
            asInt = int(base36, 36)
            return to_bech32_address(pad_left_hex(baseN(asInt, 16), 20))
