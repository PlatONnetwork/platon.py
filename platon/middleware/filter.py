...
def iter_latest_block(
    ...
    while True:
        latest_block = w3.platon.block_number
...
def get_logs_multipart(
    ...
    address: Union[Address, Bech32Address, List[Union[Address, Bech32Address]]],
    topics: List[Optional[Union[_Hash32, List[_Hash32]]]],
    max_blocks: int
) -> Iterable[List[LogReceipt]]:
    """Used to break up requests to ``platon_getLogs``
    ...
    for from_block, to_block in _block_ranges:
        ...
        yield w3.platon.get_logs(cast(FilterParams, drop_items_with_none_value(params)))
...
class RequestLogs:
    ...
    def __init__(
        ...
        address: Optional[Union[Address,
                                Bech32Address,
                                List[Union[Address, Bech32Address]]]] = None,
        ...
        if from_block is None or from_block == "latest":
            self._from_block = BlockNumber(w3.platon.block_number + 1)
    ...
    def to_block(self) -> BlockNumber:
        if self._to_block is None:
            to_block = self.w3.platon.block_number
...
class RequestBlocks:
    def __init__(self, w3: "Web3") -> None:
        self.w3 = w3
        self.start_block = BlockNumber(w3.platon.block_number + 1)
...
def block_hashes_in_range(
    ...
    for block_number in range(from_block, to_block + 1):
        yield getattr(w3.platon.get_block(BlockNumber(block_number)), "hash", None)
...
def local_filter_middleware(
    ...
    def middleware(method: RPCEndpoint, params: Any) -> RPCResponse:
        if method in NEW_FILTER_METHODS:
            ...
            if method == RPC.platon_newFilter:
                _filter = RequestLogs(w3, **apply_key_map(FILTER_PARAMS_KEY_MAP, params[0]))

            elif method == RPC.platon_newBlockFilter:
            ...
            if method == RPC.platon_getFilterChanges:
                return {"result": next(_filter.filter_changes)}
            elif method == RPC.platon_getFilterLogs:
