...
def gas_price_strategy_middleware(
    ...
    def middleware(method: RPCEndpoint, params: Any) -> RPCResponse:
        if method == 'platon_sendTransaction':
            ...
            generated_gas_price = web3.platon.generate_gas_price(transaction)
...
async def async_gas_price_strategy_middleware(
    ...
    async def middleware(method: RPCEndpoint, params: Any) -> RPCResponse:
        if method == 'platon_sendTransaction':
            transaction = params[0]
            generated_gas_price = await web3.platon.generate_gas_price(transaction)  # type: ignore
            latest_block = await web3.platon.get_block('latest')  # type: ignore
            transaction = validate_transaction_params(
                transaction, latest_block, generated_gas_price
            )
