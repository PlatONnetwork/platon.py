...
class ERC1319Registry(ABC):
    ...
    defines the methods specified in `ERC 1319 <https://github.com/platonnetwork/EIPs/issues/1319>`__.
    All of these methods are prefixed with an underscore, since they are not intended to be
    accessed directly, but rather through the methods on ``platon.pm``. They are unlikely to change,
    ...
    All of these methods must be implemented in any subclass in order to work with `platon.pm.PM`.
    ...
    def __init__(self, address: Address, w3: Web3) -> None:
        """
        Initializes the class with the on-chain address of the registry, and a platon instance
        connected to the chain where the registry can be found.

        Must set the following properties...

        * ``self.registry``: A `platon.contract` instance of the target registry.
        * ``self.address``: The address of the target registry.
        * ``self.w3``: The *platon* instance connected to the chain where the registry can be found.
        """
        pass
...
class SimpleRegistry(ERC1319Registry):
    ...
    <https://github.com/platonpm/solidity-registry>`__.
    """

    def __init__(self, address: Bech32Address, w3: Web3) -> None:
        abi = get_simple_registry_manifest()["contractTypes"]["PackageRegistry"][
            "abi"
        ]
        self.registry = w3.platon.contract(address=address, abi=abi)
    ...
    def _release(self, package_name: str, version: str, manifest_uri: str) -> bytes:
        ...
        self.w3.platon.wait_for_transaction_receipt(tx_hash)
...
class PM(Module):
    ...
    implementation of `ERC1319  <https://github.com/platonnetwork/EIPs/issues/1319>`__, set as
    its ``registry`` attribute.
    """

    def get_package_from_manifest(self, manifest: Manifest) -> Package:
        """
        Returns a `Package <https://github.com/platonpm/py-platonpm/blob/master/platonpm/package.py>`__
    ...
    def get_local_package(self, package_name: str, platonpm_dir: Path = None) -> Package:
        """
        Returns a `Package <https://github.com/platonpm/py-platonpm/blob/master/platonpm/package.py>`__
        instance built with the Manifest found at the package name in your local platonpm_dir.

        * Parameters:
            * ``package_name``: Must be the name of a package installed locally.
            * ``platonpm_dir``: Path pointing to the target platonpm directory (optional).
        """
        if not platonpm_dir:
            platonpm_dir = Path.cwd() / '_platonpm_packages'

        if not platonpm_dir.name == "_platonpm_packages" or not platonpm_dir.is_dir():
            raise PMError(f"{platonpm_dir} is not a valid ethPM packages directory.")

        local_packages = [pkg.name for pkg in platonpm_dir.iterdir() if pkg.is_dir()]
        if package_name not in local_packages:
            raise PMError(
                f"Package: {package_name} not found in {platonpm_dir}. "
                f"Available packages include: {local_packages}."
            )

        target_manifest = json.loads(
            (platonpm_dir / package_name / "manifest.json").read_text()
        )
        return self.get_package_from_manifest(target_manifest)

    def set_registry(self, address: Union[Address, Bech32Address, ENS]) -> None:
        """
        Sets the current registry used in ``platon.pm`` functions that read/write to an on-chain
        registry. This method accepts bech32 addresses or ENS names. Addresses
        must point to an on-chain instance of an ERC1319 registry implementation.

        To use an ENS domain as the address, make sure a valid ENS instance set as ``platon.ens``.

        * Parameters:
            * ``address``: Address of on-chain Registry.
        """
        if is_bech32_address(address):
            self.registry = SimpleRegistry(cast(Bech32Address, address), self.web3)
            ...
            raise PMError(
                "Expected a bech32 address or ENS name for the address, "
                f"instead received {type(address)}."
            )

    def deploy_and_set_registry(self) -> Bech32Address:
        """
        Returns the address of a freshly deployed instance of `SimpleRegistry`
        and sets the newly deployed registry as the active registry on ``platon.pm.registry``.

        To tie your registry to an ENS name, use platon's ENS module, ie.
        ...
        return to_bech32_address(self.registry.address)
    ...
    def release_package(
        ...
        Requires ``platon.PM`` to have a registry set. Requires ``platon.platon.default_account``
    ...
    def _validate_set_registry(self) -> None:
        try:
            ...
            raise PMError(
                "platon.pm does not have a set registry. "
                "Please set registry with either: "
                "platon.pm.set_registry(address) or "
                "platon.pm.deploy_and_set_registry()"
            )
        if not isinstance(self.registry, ERC1319Registry):
            raise PMError(
                "platon.pm requires an instance of a subclass of ERC1319Registry "
                "to be set as the platon.pm.registry attribute. Instead found: "
    ...
    def _validate_set_ens(self) -> None:
        if not self.web3:
            raise InvalidAddress(
                "Could not look up ENS address because no platon " "connection available"
            )
        elif not self.web3.ens:
            raise InvalidAddress(
                "Could not look up ENS address because platon.ens is " "set to None"
            )
