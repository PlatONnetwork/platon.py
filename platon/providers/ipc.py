...
def get_ipc_socket(ipc_path: str, timeout: float = 0.1) -> socket.socket:
    if sys.platform == 'win32':
        ...
        from platon._utils.windows import NamedPipe
...
def get_dev_ipc_path() -> str:  # type: ignore
    if os.environ.get('PLATON_PROVIDER_URI', ''):
        ipc_path = os.environ.get('PLATON_PROVIDER_URI')
        if os.path.exists(ipc_path):
            return ipc_path
...
class IPCProvider(JSONBaseProvider):
    logger = logging.getLogger("platon.providers.IPCProvider")
    _socket = None
