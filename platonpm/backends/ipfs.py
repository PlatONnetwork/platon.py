...
from platonpm.backends.base import (
    BaseURIBackend,
)
...
class IPFSOverHTTPBackend(BaseIPFSBackend):
    ...
    def fetch_uri_contents(self, uri: str) -> bytes:
        ...
        # Improved validation WIP @ https://github.com/platonpm/py-platonpm/pull/165
...
class DummyIPFSBackend(BaseIPFSBackend):
    ...
    - Path to manifest/contract in platonpm_spec_dir -> defined manifest/contract
    """

    def fetch_uri_contents(self, ipfs_uri: str) -> bytes:
        pkg_name = MANIFEST_URIS[ipfs_uri]
        platonpm_spec_dir = get_platonpm_spec_dir()
        pkg_contents = (platonpm_spec_dir / "examples" / pkg_name / "v3.json").read_text()
