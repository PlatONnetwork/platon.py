...
def contract_type(
    ...
    Kwargs must match fields as defined in the PlatonPM Spec (except "alias") if user
...
def _build_deployments_object(
    ...
    yield "address", to_bech32_address(address)
...
@curry
def init_manifest(
    package_name: str, version: str, manifest_version: Optional[str] = "platonpm/3"
) -> Dict[str, Any]:
    """
...
def as_package(w3: "Web3", manifest: Manifest) -> Package:
    """
    Return a Package object instantiated with the provided manifest and platon instance.
    """
