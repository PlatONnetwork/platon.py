...
from platonpm._utils.registry import (
    is_ens_domain,
)
...
def validate_registry_uri(uri: str) -> None:
    ...
    if "@" in pkg_id:
        if len(pkg_id.split("@")) != 2:
            raise PlatonPMValidationError("Registry URI: {pkg_id} is not properly escaped")
    ...
    if not pkg_name and pkg_version:
        raise PlatonPMValidationError("Registry URIs cannot provide a version without a package name.")
    if pkg_version:
...
def validate_registry_uri_authority(auth: str) -> None:
    ...
    or a valid bech32 contract address.
    """
    if ":" in auth:
        if len(auth.split(":")) != 2:
    ...
    if is_ens_domain(address) is False and not is_bech32_address(address):
        raise PlatonPMValidationError(
...
def validate_single_matching_uri(all_blockchain_uris: List[str], w3: "Web3") -> str:
    ...
    """
    from platonpm.uri import check_if_chain_matches_chain_uri
    ...
    if not matching_uris:
        raise PlatonPMValidationError("Package has no matching URIs on chain.")
