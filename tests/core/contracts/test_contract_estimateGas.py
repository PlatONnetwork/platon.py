...
def math_contract(web3,
    ...
    web3.isBech32Address(contract_address)
...
def fallback_function_contract(web3,
                               ...
                               wait_for_transaction,
                               address_conversion_func):
    fallback_contract = web3.platon.contract(
        abi=FALLBACK_FUNCTION_ABI,
        bytecode=FALLBACK_FUNCTION_CODE,
        bytecode_runtime=FALLBACK_FUNCTION_RUNTIME
    )
    deploy_txn = fallback_contract.constructor().transact({'from': web3.platon.coinbase})
    deploy_receipt = web3.platon.wait_for_transaction_receipt(deploy_txn)
...
def payable_tester_contract(web3, PayableTesterContract, address_conversion_func):
    deploy_txn = PayableTesterContract.constructor().transact({'from': web3.platon.coinbase})
    deploy_receipt = web3.platon.wait_for_transaction_receipt(deploy_txn)
...
def test_estimateGas_block_identifier_unique_estimates(web3, math_contract, transact):
    ...
    web3.platon.wait_for_transaction_receipt(txn_hash)
