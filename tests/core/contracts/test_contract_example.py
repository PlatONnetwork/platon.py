# This file is used by the documentation as an example of how to write unit tests with platon.py
import pytest
...
@pytest.fixture
def tester_provider():
    return PlatonTesterProvider()


@pytest.fixture
def platon_tester(tester_provider):
    return tester_provider.platon_tester
...
@pytest.fixture
def foo_contract(platon_tester, w3):
    ...
    deploy_address = platon_tester.get_accounts()[0]
    ...
    FooContract = w3.platon.contract(abi=abi, bytecode=bytecode)
    # issue a transaction to deploy the contract.
    tx_hash = FooContract.constructor().transact({
        'from': deploy_address,
    })
    # wait for the transaction to be mined
    tx_receipt = w3.platon.wait_for_transaction_receipt(tx_hash, 180)
...
def test_can_update_greeting(w3, foo_contract):
    ...
    tx_hash = foo_contract.functions.setBar(
        ...
        'from': w3.platon.accounts[1],
    })
    w3.platon.wait_for_transaction_receipt(tx_hash, 180)
...
def test_updating_greeting_emits_event(w3, foo_contract):
    ...
    receipt = w3.platon.wait_for_transaction_receipt(tx_hash, 180)
