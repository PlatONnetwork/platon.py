...
def deploy_contracts(web3, contract, wait_for_transaction):
    for i in range(25):
        ...
        yield web3.platon.get_transaction_receipt(tx_hash)['contractAddress']
...
def pad_with_transactions(w3):
    ...
    for tx_count in range(random.randint(0, 10)):
        ...
        value = 50 + tx_count
        w3.platon.send_transaction({'from': _from, 'to': _to, 'value': value})
...
def single_transaction(w3):
    ...
    _to = accounts[random.randint(0, len(accounts) - 1)]
    value = 50
    tx_hash = w3.platon.send_transaction({'from': _from, 'to': _to, 'value': value})
    return tx_hash
...
def test_event_filter_new_events(
    ...
    while web3.platon.block_number < 50:
...
@pytest.mark.xfail(reason="Suspected platon-tester bug")
def test_block_filter(web3):
    ...
    assert len(block_filter.get_new_entries()) == web3.platon.block_number


def test_transaction_filter_with_mining(web3):

    transaction_filter = web3.platon.filter("pending")
...
def test_transaction_filter_without_mining(
    ...
    web3.providers[0].platon_tester.auto_mine_transactions = False
    transaction_filter = web3.platon.filter("pending")
