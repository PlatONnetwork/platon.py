...
def test_createIndirect(value, expected, web3):
    actual = web3.platon.iban.createIndirect(value).toString()
...
def test_fromAddress(value, expected, web3):
    ...
    if isinstance(expected, type) and issubclass(expected, Exception):
        with pytest.raises(expected):
            web3.platon.iban.fromAddress(value).toString()
        return

    actual = web3.platon.iban.fromAddress(value).toString()
    assert actual == expected
...
def test_isValid(value, expected, web3):
    actual = web3.platon.iban.isValid(value)
    assert actual is expected

    iban = web3.platon.iban(value)
...
def test_toAddress(value, expected, web3):
    actual = web3.platon.iban(value).address()
    assert actual == expected
