...
def test_pm_init_with_minimal_manifest(w3):
    owned_manifest = get_platonpm_spec_manifest('owned', 'v3.json')
...
def test_get_contract_factory_raises_insufficient_assets_error(w3):
    insufficient_owned_manifest = get_platonpm_spec_manifest('owned', 'v3.json')
...
def test_get_contract_factory_with_valid_owned_manifest(w3):
    owned_manifest = get_platonpm_local_manifest('owned', 'with_contract_type_v3.json')
...
def test_get_contract_factory_with_valid_safe_math_lib_manifest(w3):
    safe_math_lib_manifest = get_platonpm_spec_manifest('safe-math-lib', 'v3.json')
...
def test_get_contract_factory_with_valid_escrow_manifest(w3):
    escrow_manifest = get_platonpm_spec_manifest("escrow", "v3.json")
    ...
    safe_send_tx_receipt = w3.platon.wait_for_transaction_receipt(safe_send_tx_hash)
    ...
    escrow_tx_hash = linked_escrow_factory.constructor(w3.platon.accounts[0]).transact()
    ...
    assert escrow_instance.functions.sender().call() == w3.platon.accounts[0]


def test_deploy_a_standalone_package_integration(w3):
    standard_token_manifest = get_platonpm_local_manifest("standard-token", "with_bytecode_v3.json")
    ...
    erc20 = w3.platon.contract(address=address, abi=ERC20.abi)
...
@pytest.fixture
def tmp_platonpmdir(tmp_path):
    owned_manifest = get_platonpm_spec_manifest("owned", "v3.json")
    platonpmdir = tmp_path / '_platonpm_packages'
    platonpmdir.mkdir()
    owned_dir = platonpmdir / 'owned'
    owned_dir.mkdir()
...
def test_get_local_package(w3, tmp_platonpmdir):
    pkg = w3.pm.get_local_package("owned", tmp_platonpmdir)
    assert isinstance(pkg, Package)
    assert pkg.name == "owned"


def test_get_local_package_with_invalid_platonpmdir(w3, tmp_path):
    invalid_platonpmdir = tmp_path / 'invalid'
    invalid_platonpmdir.mkdir()
    with pytest.raises(PMError, match="not a valid ethPM packages directory."):
        w3.pm.get_local_package("owned", invalid_platonpmdir)


def test_get_local_package_with_uninstalled_package(w3, tmp_platonpmdir):
    with pytest.raises(PMError, match="Package: safe-math not found in "):
        w3.pm.get_local_package("safe-math", tmp_platonpmdir)
