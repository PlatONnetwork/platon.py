...
def test_user_code_with_fixture(greeter):
    ...
    assert isinstance(greeter_instance, platon.contract.Contract)
...
# LIBRARY
@pytest.fixture
def safe_math(deployer, platonpm_spec_dir):
    safe_math_manifest_path = platonpm_spec_dir / "examples" / "safe-math-lib" / "v3.json"
...
def test_escrow_deployer_unlinked(escrow_deployer):
    with pytest.raises(DeployerError):
        escrow_deployer.deploy("Escrow", escrow_deployer.package.w3.platon.accounts[0])
