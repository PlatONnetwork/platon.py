...
def test_linker_with_from(escrow_deployer, w3):
    escrow_strategy = linker(
        deploy("SafeSendLib"),
        link("Escrow", "SafeSendLib"),
        deploy("Escrow", w3.platon.accounts[0], transaction={"from": w3.platon.accounts[5]}),
    )
    escrow_deployer.register_strategy("Escrow", escrow_strategy)
    linked_escrow_package = escrow_deployer.deploy("Escrow")
    escrow_instance = linked_escrow_package.deployments.get_instance("Escrow")
    assert escrow_instance.functions.sender().call() == w3.platon.accounts[5]


def test_linker_with_callback(escrow_deployer, w3):
    sender = w3.platon.accounts[0]
    recipient = w3.platon.accounts[5]
    ...
    def callback_fn(package):
        ...
        w3.platon.wait_for_transaction_receipt(tx_hash)
    ...
    escrow_strategy = linker(
        ...
        deploy(
            "Escrow",
            recipient,
            transaction={"from": sender, "value": w3.toVon("1", "ether")},
        ),
        run_python(callback_fn),
    )
    escrow_deployer.register_strategy("Escrow", escrow_strategy)
    assert w3.platon.get_balance(recipient) == w3.toVon("1000000", "ether")
    linked_escrow_package = escrow_deployer.deploy("Escrow")
    escrow_instance = linked_escrow_package.deployments.get_instance("Escrow")
    assert escrow_instance.functions.sender().call() == sender
    assert w3.platon.get_balance(recipient) == w3.toVon("1000001", "ether")
