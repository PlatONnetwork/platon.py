...
def generate_go_platon_fixture(destination_dir):
...
def verify_chain_state(web3, chain_data):
    receipt = web3.platon.wait_for_transaction_receipt(chain_data['mined_txn_hash'])
    latest = web3.platon.get_block('latest')
...
def setup_chain_state(web3):
    ...
    # Math Contract
    #
    math_contract_factory = web3.platon.contract(
        abi=MATH_ABI,
    ...
    # Emitter Contract
    #
    emitter_contract_factory = web3.platon.contract(
    ...
    block_with_log = web3.platon.get_block(txn_receipt_with_log['blockHash'])
    print('BLOCK_HASH_WITH_LOG:', block_with_log['hash'])

    #
    # Revert Contract
    #
    revert_contract_factory = web3.platon.contract(
    ...
    txn_hash_normal_function = revert_contract.functions.normalFunction().transact(
        {'gas': 320000, 'from': web3.platon.coinbase}
    )
    ...
    block_hash_revert_with_msg = web3.platon.get_block(txn_receipt_revert_with_msg['blockHash'])
    ...
    block_hash_revert_no_msg = web3.platon.get_block(txn_receipt_revert_with_no_msg['blockHash'])
    ...
    empty_block = web3.platon.get_block(empty_block_number)
...
if __name__ == '__main__':
    ...
    generate_go_platon_fixture(fixture_dir)
