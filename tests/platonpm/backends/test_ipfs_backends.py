...
@pytest.fixture
def owned_manifest_path(platonpm_spec_dir):
    return platonpm_spec_dir / "examples" / "owned" / "v3.json"
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("ipfs:/QmTKB75Y73zhNbD3Y73xeXGjYrZHmaXXNxoZqGCagu7r8u", True),
        ("ipfs://QmTKB75Y73zhNbD3Y73xeXGjYrZHmaXXNxoZqGCagu7r8u", True),
        ("http://raw.githubusercontent.com/platonpm/py-platonpm#0x123", False),
        ("https://raw.githubusercontent.com/platonpm/py-platonpm#0x123", False),
        (
            "bzz://679bde3ccb6fb911db96a0ea1586c04899c6c0cc6d3426e9ee361137b270a463",
            False,
        ),
        ("ercxxx://packages.platon/owned?version=1.0.0", False),
    ),
)
...
def test_get_uri_backend_with_env_variable(dummy_ipfs_backend, monkeypatch):
    monkeypatch.setenv(
        "ETHPM_IPFS_BACKEND_CLASS", "platonpm.backends.ipfs.LocalIPFSBackend"
    )
    backend = get_ipfs_backend()
    assert isinstance(backend, LocalIPFSBackend)


def test_pin_assets_to_dummy_backend(dummy_ipfs_backend, platonpm_spec_dir, owned_manifest_path):
    # Test pinning a file
    backend = get_ipfs_backend()
    ...
    dir_data = backend.pin_assets(platonpm_spec_dir / "examples" / "standard-token" / "contracts")
