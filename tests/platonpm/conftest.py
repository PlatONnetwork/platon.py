...
from platonpm._utils.chains import (
    create_block_uri,
)
...
def fetch_manifest(name, version):
    return get_platonpm_spec_manifest(name, version)


def fetch_manifest_path(name, version):
    platonpm_spec_dir = get_platonpm_spec_dir()
    return platonpm_spec_dir / 'examples' / name / version
...
def piper_coin_manifest():
    return get_platonpm_spec_manifest("piper-coin", "v3.json")
...
@pytest.fixture
def owned_contract(platonpm_spec_dir):
    return (platonpm_spec_dir / "examples" / "owned" / "contracts" / "Owned.sol").read_text()
...
@pytest.fixture
def escrow_package(deployer, w3, platonpm_spec_dir):
    escrow_manifest = platonpm_spec_dir / "examples" / "escrow" / "v3.json"
    ...
    escrow_strategy = linker.linker(
        ...
        linker.deploy("Escrow", w3.platon.accounts[0]),
    )
...
def manifest_with_no_matching_deployments(w3, tmpdir, safe_math_manifest):
    ...
    block = w3.platon.get_block("earliest")
