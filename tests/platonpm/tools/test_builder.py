...
from platonpm.backends.ipfs import (
    get_ipfs_backend,
)
...
from platonpm.tools.builder import (
    as_package,
    authors,
    build,
    build_dependency,
...
BASE_MANIFEST = {"name": "package", "manifest": "platonpm/3", "version": "1.0.0"}


@pytest.fixture
def owned_package(platonpm_spec_dir):
    manifest = get_platonpm_spec_manifest("owned", "v3.json")
    # source_id missing `./` prefix in platonpm-spec ("Owned.sol"/"./Owned.sol" though both are valid)
    source_obj = manifest['sources'].pop('Owned.sol')
    updated_manifest = assoc_in(manifest, ['sources', './Owned.sol'], source_obj)

    compiler = get_platonpm_local_manifest("owned", "output_v3.json")["contracts"]
    contracts_dir = platonpm_spec_dir / "examples" / "owned" / "contracts"
...
@pytest.fixture
def standard_token_package(platonpm_spec_dir):
    standard_token_dir = platonpm_spec_dir / "examples" / "standard-token"
    manifest = get_platonpm_spec_manifest("standard-token", "v3.json")
    compiler = get_platonpm_local_manifest("standard-token", "output_v3.json")["contracts"]
...
MINIFIED_MANIFEST = (
    '{"manifest":"platonpm/3","name":"package","version":"1.0.0"}'
)
...
def test_builder_with_manifest_validation():
    with pytest.raises(PlatonPMValidationError, match="_invalid_package_name"):
        build(
            {},
...
@pytest.fixture
def escrow_package(w3, deployer, platonpm_spec_dir):
    manifest = platonpm_spec_dir / "examples" / "escrow" / "v3.json"
    escrow_deployer = deployer(manifest)
    escrow_strategy = linker(
        deploy("SafeSendLib"),
        link("Escrow", "SafeSendLib"),
        deploy("Escrow", w3.platon.accounts[0]),
    )
...
def test_builder_deployment_type_complex(escrow_package):
    ...
    manifest = build(
        ...
        manifest_version("platonpm/3"),
        escrow_dep_type(
            block_uri="blockchain://1111111111111111111111111111111111111111111111111111111111111111/block/1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef",
...
def test_builder_with_invalid_uri():
    with pytest.raises(
        PlatonPMValidationError, match="is not a supported content-addressed URI"
    ):
        build(
            {},
            package_name("package"),
