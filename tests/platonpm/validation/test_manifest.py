...
def test_validate_manifest_invalidates(invalid_manifest):
    with pytest.raises(PlatonPMValidationError, match="Manifest invalid for schema"):
...
def test_validate_manifest_deployments_catches_missing_contract_type_references(
    ...
    with pytest.raises(PlatonPMValidationError, match="Manifest missing references to contracts"):
...
def test_validate_manifest_version_validates_version_three_string():
    assert validate_manifest_version("platonpm/3") is None
